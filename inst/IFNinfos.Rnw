...
<<OptionsGenerales_ancien, include=F, eval=F>>=
opts_chunk$set(concordance=T,include=T,tidy=F, warning=F, comment=F, error=F, echo=F, message=F)
...
print(xtable(acct$tab, caption = "Accroissement sur le diamètre (en cm/an) par essence et classe de diamètre.", digits=c(rep(0,2),rep(2,3)), label = "IFNacctDtab", caption.placement = "top"), include.rownames=FALSE)
@
